import { Triangle, CheckCircle2, Clock, XCircle, ArrowUpRight, Globe } from "lucide-react";

export default function VercelPage() {
  return (
    <div className="p-8 px-12 w-full max-w-7xl mx-auto flex flex-col gap-10">
      <div className="flex items-center justify-between relative">
        <div className="absolute -left-6 top-2 bottom-2 w-1 bg-gradient-to-b from-healthy-500 to-transparent" />
        <div>
          <h1 className="text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-neutral-50 to-neutral-400 tracking-tight uppercase mb-2">Vercel Deployments</h1>
          <p className="text-neutral-400 tracking-wide">Track your application builds and deployments.</p>
        </div>
      ...
      <div className="bg-neutral-900/40 backdrop-blur-md border border-neutral-800/60 shadow-xl rounded-2xl overflow-hidden relative">
        ...
        <div className="p-6 border-b border-neutral-800/50 flex items-center justify-between bg-neutral-900/30 relative z-10">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-neutral-50 tracking-wide">Recent Deployments</h2>
          </div>
          <span className="text-xs font-mono px-3 py-1 bg-neutral-800/80 rounded-md border border-neutral-700 text-neutral-400 uppercase tracking-widest">nexus-ops-web</span>
        </div>
        
        <div className="divide-y divide-neutral-800/40 relative z-10">
          {[
            { id: "dpl_9x8A", status: "ready", env: "Production", time: "10m", branch: "main", commit: "feat: redesign side nav" },
            { id: "dpl_2b7C", status: "building", env: "Preview", time: "Just now", branch: "feat/new-ui", commit: "update globals.css" },
            { id: "dpl_4y6E", status: "error", env: "Preview", time: "1h", branch: "fix/auth", commit: "chore: update deps" },
            { id: "dpl_1a3D", status: "ready", env: "Production", time: "2d", branch: "main", commit: "Initial commit" },
          ].map((dpl) => (
            <div key={dpl.id} className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-neutral-800/40 transition-all duration-200 group relative">
              {dpl.status === "building" && (
              ...
              <div className="flex items-start sm:items-center gap-5">
                ...
                <div>
                  <div className="flex items-center gap-3 mb-1.5">
                    <span className={`font-bold tracking-wide ${dpl.env === "Production" ? "text-neutral-100" : "text-neutral-400"}`}>
                      {dpl.env}
                    </span>
                  ...
                  <p className="text-sm text-neutral-400 group-hover:text-neutral-300 transition-colors">{dpl.commit}</p>
                </div>
              </div>
              
              <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-8 sm:pl-0 pl-11">
                <div className="flex flex-col sm:items-end gap-1">
                  <span className="text-sm font-mono text-neutral-400">{dpl.time} ago</span>
                  <span className="text-xs font-mono text-neutral-600 bg-neutral-800/50 px-2 rounded">{dpl.id}</span>
                </div>
